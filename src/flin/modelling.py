...
def strict_train_tune(all_laps_clean: pd.DataFrame):
    ...
    hg_grid = {
        ...
        "reg__l2_regularization":[0.0, 1e-2]
    }
    ...
    lgbm = LGBMRegressor(random_state=42, categorical_feature=cat_idx,n_jobs=1)
    lgb_grid = {
    ...
    return best_hgbr, best_lgb, full_pre


# ──────────────────────────────────────────────────────────────
# CELL-7 · strict evaluation on hold-out races
# ──────────────────────────────────────────────────────────────
def evaluate_holdout(best_hgbr, best_lgb, full_pre, all_laps_clean):
    df = (all_laps_clean.copy()
            .dropna(subset=["LapTimeSeconds"])
            .assign(IsFirstRaceOfSeason=lambda x: (x["Round"] == 1).astype(int)))
