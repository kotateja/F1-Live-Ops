...
def drop_deleted_and_fastf1(df: pd.DataFrame) -> pd.DataFrame:
    # Ensure boolean types, filling NaNs with False
    deleted = df["Deleted"].fillna(False).astype(bool)
    fastf1_generated = df["FastF1Generated"].fillna(False).astype(bool)

    mask = deleted | fastf1_generated
    n = mask.sum()
